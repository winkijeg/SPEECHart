...
%%L global first_contact;				%modified by Yohan & Majid; Nov 30, 99
%%L not used anywhere but here


%%L global kkk t_initial t_transition t_final theta theta_ll dist_lip hyoid_movment dist_hyoid Hyoid_corps lar_ar_mri tongue_lar_mri pharynx_mri
...
if TSObj.kkk==0	%modified by Yohan & Majid; Nov 30, 99
    ...
    TSObj.UU.pharynx_mri = [];
    TSObj.UU.lowerteeth=[];
    length_ttout=0;
    X0_seq=[];
    Y0_seq=[];
    old_t=TSObj.t_initial;
end
...
if length(TSObj.ttout)>length_ttout
    length_ttout=length_ttout+1;
    TSObj.UU.X_origin(length_ttout)=TSObj.cont.X_condyle;
    TSObj.UU.Y_origin(length_ttout)=TSObj.cont.Y_condyle;
    TSObj.UU.lowerlip(length_ttout,:)=[TSObj.cont.lowerlip(1,:) TSObj.cont.lowerlip(2,:)];
    TSObj.UU.upperlip(length_ttout,:)=[TSObj.cont.upperlip(1,:) TSObj.cont.upperlip(2,:)];
    TSObj.UU.lowerteeth(length_ttout,:)=[TSObj.cont.lowerteeth(1,:) TSObj.cont.lowerteeth(2,:)];
...
if t>=t_affiche                         %      |
    ...
    plot(lowerteeth(1,:),lowerteeth(2,:),['-' couleurs(index_coul)]);
    plot(lowerlip(1,:),lowerlip(2,:),['-' couleurs(index_coul)]);
